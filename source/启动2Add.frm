     ...
     If Right(Command, 1) = """" Then
      For cli = 1 To Len(Command)
       If Mid(Command, cli, 1) = """" Then
          myWriteINI App.Path + "\SmM_Start.dat", "Start", "AddFile", Mid(Command, cli + 1, Len(Command) - cli - 1)
    
        Exit For
        End If
    Next
    Else
                 myWriteINI App.Path + "\SmM_Start.dat", "Start", "AddFile", Command
  
    End If
           Ly.SetDWORDValue "HKEY_LOCAL_MACHINE\SOFTWARE\H2O Networks\Snowman Media ilxz 4", "AddFile", True
Else:
