...
Begin VB.Form frmThumbs 
   Caption         =   "浏览  － Snowman Media Pictures Browser  1.0"
   ClientHeight    =   4290
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   8880
...
Dim sPath   As String
   ...
   ' If Not bDontShowBrowser Then
        sPath = BrowseForFolder(Me.hWnd, "请选择你要浏览的文件夹, Snowman Media Pictures Browser  1.0 将为你打开它.", sPath)
    'End If
    If Len(sPath) > 0 Then
...
Dim sText   As String
    ...
    lblCount.Caption = "本文件夹共有图片数:" & Format$(lFilCnt, "#,##0")
    If Len(sPath) > 0 Then
        Call StartProgress
        ...
        lblCount.Caption = "本文件夹共有图片数:" & Format$(lPicCnt, "#,##0")
