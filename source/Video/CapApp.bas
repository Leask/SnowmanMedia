...
'****************************************************************
'* FUNCTION vbGetAvailableMBytes()
'* ===============
'* adapted from a routine in the "GetFree" example by Ray Mercer
'* on Shrinkwrap Visual Basic Website (http://i.am/shrinkwrapvb)
'* This routine will return the number of free bytes on the
'* specified drive (does not handle drive partitions over
'* 2GB)
'*
'* INPUTS:
'* sPath - (see notes in vbGetAvailableBytesAsString function in GetFree example)
'*
'* RETURNS:
'* free disk space in Mbytes
'****************************************************************
Public Function vbGetAvailableMBytes(ByVal sPath As String) As Long
    ...
    vbGetAvailableMBytes = (lSpc * lBps * lNfc) / ONE_MEGABYTE
